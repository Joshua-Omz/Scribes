...
Supports both chat completion (modern instruction models) and text generation (completion models).
...
class HFInferenceService:
    """
    Singleton service for HuggingFace LLM inference.
    
    Supports:
    - Chat completion (chat_completion) for instruction-tuned models (Llama-3, Mistral-Instruct)
    - Text generation (text_generation) for completion models (GPT-2, Llama-2-base)
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            cls._instance = super(HFInferenceService, cls).__new__(cls)
    ...
    def __init__(self):
        ...
        self.api_mode = settings.hf_api_mode  # 'chat' or 'text'
        
        logger.info(
            f"Initializing HFInferenceService: mode={'API' if self.use_api else 'LOCAL'}, "
            f"model={self.model_name}, api_mode={self.api_mode}"
        )
        ...
        try:
            ...
            logger.info("HFInferenceService initialized successfully")
    ...
    def generate_from_messages(
        ...
        """
        Generate text from chat messages (for instruction-tuned models).
        
        Args:
            messages: List of message dicts with 'role' and 'content' keys
                     Example: [{"role": "system", "content": "..."}, {"role": "user", "content": "..."}]
            max_tokens: Max tokens to generate (default from config: 400)
            temperature: Randomness 0-1 (default: 0.2 for factual)
            top_p: Nucleus sampling (default: 0.9)
        
        Returns:
            Generated answer text (cleaned)
        
        Raises:
            GenerationError: If generation fails after retries
        """
        # Use config defaults if not specified
        max_tokens = max_tokens or settings.assistant_max_output_tokens
        temperature = temperature or settings.hf_generation_temperature
        top_p = top_p or settings.assistant_model_top_p
        
        # Validate inputs
        if not messages or len(messages) == 0:
            raise GenerationError("Cannot generate from empty messages list")
        
        logger.info(
            f"Generating from {len(messages)} messages: max_tokens={max_tokens}, temp={temperature}, "
        ...
        try:
            if self.use_api:
                answer = self._generate_api_chat(messages, max_tokens, temperature, top_p)
            else:
                answer = self._generate_local_chat(messages, max_tokens, temperature, top_p)
    ...
    def _generate_api_chat(
        ...
        try:
            ...
            logger.error(f"HF API chat_completion failed: {e}", exc_info=True)
            raise GenerationError(f"API chat_completion call failed: {str(e)}")
    ...
    def _generate_local_chat(
        ...
        """Generate using local model with chat template."""
        try:
            import torch
            
            # Apply chat template to convert messages to prompt
            if hasattr(self._tokenizer, "apply_chat_template"):
                # Modern transformers with chat template support
                ...
                # Fallback: manual formatting (simple concatenation)
                prompt = ""
                for msg in messages:
                ...
                prompt += "Assistant: "  # Prompt for generation
            ...
            inputs = self._tokenizer(
                ...
                return_tensors="pt",
                truncation=True,
                max_length=settings.assistant_model_context_window - max_tokens
            ...
            logger.error(f"Local chat generation failed: {e}", exc_info=True)
    ...
    def generate_from_prompt(
        ...
        """
        Generate text from raw prompt (for completion models - LEGACY).
        
        This method is for backward compatibility with completion models.
        For instruction-tuned models, use generate_from_messages() instead.
        ...
        try:
            if self.use_api:
                answer = self._generate_api_text(prompt, max_new_tokens, temperature, top_p, repetition_penalty)
            else:
                answer = self._generate_local_text(prompt, max_new_tokens, temperature, top_p, repetition_penalty)
    ...
    def _generate_api_text(
        ...
        try:
            ...
            logger.error(f"HF API text_generation failed: {e}", exc_info=True)
    ...
    def _generate_local_text(
        ...
        """Generate using local model (GPU/CPU) - raw text completion."""
        try:
            import torch
...
def get_inference_service() -> HFInferenceService:
    """
    Get or create global HFInferenceService instance.
    
    Returns:
        HFInferenceService singleton
    """
    global _inference_service_instance
    if _inference_service_instance is None:
        _inference_service_instance = HFInferenceService()
    return _inference_service_instance


# Backward compatibility aliases
get_textgen_service = get_inference_service
